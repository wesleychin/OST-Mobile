            ...
            _searchInput.on('keyup', function(e) {
                ...
                if (ctrl.tagging.isActivated && ctrl.search.length > 0) {
                    ...
                    if (ctrl.tagging.fct !== undefined) {
                        tagItems = ctrl.$filter('filter')(items, {
                            'isTag': true
                        });
...
angular.module("ui.select").run(["$templateCache", function($templateCache) {
    ...
    $templateCache.put("select2/match.tpl.html", "<a class=\"select2-choice ui-select-match\" ng-class=\"{\'select2-default\': $select.isEmpty()}\" ng-click=\"$select.activate()\"><span class=\"select2-chosen\" ng-class=\"{true :\'dropdow-placeholder-selected\' }[!$select.isEmpty() && !$select.open]\"><span>{{$select.placeholder}}</span></span> <span ng-hide=\"$select.isEmpty() || $select.open\" class=\"select2-chosen\" ng-transclude=\"\"></span> <abbr ng-if=\"$select.allowClear && !$select.isEmpty()\" class=\"select2-search-choice-close\" ng-click=\"$select.select(undefined)\"></abbr> <span class=\"select2-arrow ui-select-toggle\" ng-click=\"$select.toggle($event)\"><span class=\"fa \" ng-class=\"{\'fa-chevron-up\': $select.open, \'fa-chevron-down\': !$select.open }\" ></span></span><div class=\"form-elem-half-border\"  ng-class=\"{\'form-elem-half-border-nonempty\': !$select.isEmpty() }\"></div></a>");
